...
class PredictionPipeline:
    def __init__(self, pipeline_config:PredictionPipelineConfig()):
    ...
    def write_file(self, dataframe:DataFrame, file_path:str)->bool:
        try:
            ...
            file_path= self.get_pyspark_s3_file_path(dir=file_path)
            print(file_path)
    ...
    def is_vaid_file(self, file_path)->bool:
        try:
    ...
    def get_valid_files(self, file_paths: List[str])->Tuple[List[str], List[str]]:
        """
        This function will return tuple of valid file name list and invalid file name list
        """
        try:
            valid_file_pathhs:List[str]=[]
            invalid_file_paths:List[str]=[]

            for file_path in file_paths:
                is_valid= self.is_vaid_file(file_path=file_path)
                if is_valid:
                    valid_file_pathhs.append(file_path)
                else:
                    invalid_file_paths.append(file_path)

            return valid_file_pathhs, invalid_file_paths
        except Exception as e:
            raise FinanceException(e,sys)
    ...
    def start_batch_prediction(self):
        try:
            ...
            for valid_file in valid_files:
                logger.info(f"Strarting Prediction of File:{valid_file}")
                ...
                required_columns= self.schema.required_columns+[self.schema.prediction_label_column_name]
                logger.info(f"saving required colums:{required_columns}")
